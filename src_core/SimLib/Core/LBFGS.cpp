...
template<typename Scalar>
Scalar LBFGS<Scalar>::computeDefaultScalingFront(std::function<void(Vector &v)> applyH0) const
{
	Vector H0y = m_y.front();
	applyH0(H0y);
	Scalar scaling = m_s.front().dot(m_y.front()) / m_y.front().dot(H0y); // see "updating Quasi-Newton matrices with Limited Storage" page 781
	return scaling;
}
template<typename Scalar>
